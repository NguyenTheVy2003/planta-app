const PlantPostModle = require('./model');
...
const getAll = async (size, page) => {
    try {
        ...
        const product = await PlantPostModle.find()
            // .sort(sort)
            // .skip(skip)
            // .limit(size);
        return product;
...
const getOneById = async (id) => {
    try {
        const product = await PlantPostModle.findById(id);
        return product;
    } catch (error) {
...
const searchByName = async (name) => {
    try {
        ...
        const product = await PlantPostModle.find({ name: { $regex: name, $options: 'i' } });
        return product;
    } catch (error) {
...
const addNews = async (data) => {
    try {
        ...
        const product = new PlantPostModle({
            name, price, origin, size, image, status
        });
        await product.save();
    } catch (error) {
...
const remove = async (id) => {
    try {
      await PlantPostModle.findByIdAndDelete(id);
    } catch (error) {
        console.log(error);
