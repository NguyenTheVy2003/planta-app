const ProductMoel = require('./model');
...
const getAll = async (size, page) => {
    try {
        ...
        const product = await ProductMoel.find()
            // .sort(sort)
            // .skip(skip)
            // .limit(size);
        return product;
...
const searchByName = async (name) => {
    try {
        ...
        const product = await ProductMoel.find({ name: { $regex: name, $options: 'i' } });
        return product;
    } catch (error) {
...
const remove = async (id) => {
    try {
      await ProductMoel.findByIdAndDelete(id);
    } catch (error) {
        console.log(error);
